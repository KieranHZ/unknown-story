    ...
    TriggerEvent('skinchanger:getSkin', function(skin)
        if skin.sex == 0 then
            ...
            if job == 'bullet_wear' then
                SetPlayerMaxArmour(playerPed, 200)
                SetPedComponentVariation(playerPed, 9, 27, 9, 2)
                AddArmourToPed(playerPed, 200)
                SetPedArmour(playerPed, 200)
                PlaySoundFrontend(playerPed, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
            end
        else
...
function OpenCloakroomMenu()
    ...
    local elements = {
        ...
        { label = _U('undercover_wear'), value = 'undercover_wear' },
    }
                    ...
                    if Config.MaxInService ~= -1 then
                        ...
                        ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
                            if isInService then
                                ...
                                TriggerServerEvent('esx_service:disableService', 'police', PlayerData.identifier)
                                ESX.ShowNotification(_U('service_out'))
                            end
                        end, 'police', PlayerData.identifier)
                    end
                end
                        ...
                        if not isInService then
                            ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
                                if not canTakeService then
                                    ...
                                    ESX.ShowNotification("Appuyez sur ~g~F6 ~w~pour ouvrir le ~b~Menu police")
                                end
                            end, 'police', PlayerData.identifier)
                        else
                            serviceOk = true
...
function OpenArmoryMenu(station)
    ...
    if Config.EnableArmoryManagement then
        ...
        table.insert(elements, { label = _U('nogilet_wear'), value = 'nogilet_wear' })
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory',
            {
                ...
                elseif data.current.value == 'bullet_wear' then
                    setUniform(data.current.value, playerPed)
                elseif data.current.value == 'gilet_wear' then
                    setUniform(data.current.value, playerPed)
                elseif data.current.value == 'nogilet_wear' then
                    setUniform('nogilet_wear', playerPed)
                    setUniform('nobullet_wear', playerPed)
                end
            end, function(data, menu)
                menu.close()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
            { css = 'unknownstory',
              ...
              elements = {
                  ...
                  { label = _U('shield'), value = 'shield_spawner' },
                  { label = 'Mettre/Enlever radar mobile', value = 'spawn_radar' },
              }
            }, function(data, menu)
                ...
                elseif data.current.value == 'spawn_radar' then

                    TriggerEvent('esx_policejob:POLICE_radar')


                elseif data.current.value == 'shield_spawner' then

                    --shield_spawner()
                    TriggerEvent('police:shield_spawner')
                    ...
                    ESX.UI.Menu.Open(
                            ...
                            { css = 'unknownstory',
                              ...
                              elements = {
                                  ...
                                  { label = _U('cible_homme'), value = 'prop_range_target_01' },
                                  { label = _U('cible_fine'), value = 'prop_range_target_02' },
                                  { label = _U('cible_rondes'), value = 'prop_range_target_03' },
                                  { label = _U('lampe_sol'), value = 'prop_construcionlamp_01' },
...
function OpenBuyWeaponsMenu()
    ...
    for _, v in ipairs(Config.AuthorizedWeapons[PlayerData.job.grade_name]) do
