...
function OpenCloakroomMenu()
	...
	elseif grade == 'novice' then
		table.insert(elements, { label = _U('job_wear'), value = 'novice_wear' })
				...
				if Config.MaxInService ~= -1 then
					ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
						if isInService then
							...
							TriggerServerEvent('esx_service:disableService', 'cardealer', PlayerData.identifier)
							ESX.ShowNotification(_U('service_out'))
						end
					end, 'cardealer', PlayerData.identifier)
				end
			end
...
AddEventHandler('esx_vehicleshop:hasExitedMarker', function(zone)
	if not IsInShopMenu and not isInShopMenu then
		ESX.UI.Menu.CloseAll()
	end
	CurrentAction = nil
end)
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_phone:removeNumber', 'cardealer')
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'cardealer' then
			...
			if not isInMarker and HasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_vehicleshop:hasExitedMarker', currentZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction == nil then
			Citizen.Wait(500)
		elseif not playerInService or playerInService then
		...
		if playerInService then
			if CurrentAction == 'shop_menu' and playerInService then
				ESX.ShowHelpNotification(CurrentActionMsg)
				if IsControlJustReleased(0, Keys['E']) then
			...
			elseif CurrentAction == 'reseller_menu' and playerInService then
				ESX.ShowHelpNotification(CurrentActionMsg)
				if IsControlJustReleased(0, Keys['E']) then
					OpenResellerMenu()
				end
			elseif CurrentAction == 'give_back_vehicle' and playerInService then
				ESX.ShowHelpNotification(CurrentActionMsg)
				if IsControlJustReleased(0, Keys['E']) then
			...
			elseif CurrentAction == 'resell_vehicle' and playerInService then
				ESX.ShowHelpNotification(CurrentActionMsg)
				if IsControlJustReleased(0, Keys['E']) then
			...
			elseif CurrentAction == 'vehiclespawn_menu' and playerInService then
				ESX.ShowHelpNotification(CurrentActionMsg)
				if IsControlJustReleased(0, Keys['E']) then
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('cardealer:getStockItems', function(inventory)
			...
			{ css = 'unknownstory',
			  ...
			  elements = elements
				...
				if data.current.type == 'item_standard' then
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
						...
						if count == nil then
							...
							menu2.close()
							menu.close()
							TriggerServerEvent('cardealer:getStockItem', data.current.type, itemName, count)
				...
				elseif data.current.type == 'item_weapon' then
					...
					TriggerServerEvent('cardealer:getStockItem', data.current.type, data.current.value, data.current.ammo)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('cardealer:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
			{ css = 'unknownstory',
			  ...
			  elements = elements
				...
				if data.current.type == 'item_standard' then
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
						...
						if count == nil then
							...
							menu2.close()
							menu.close()
							TriggerServerEvent('cardealer:putStockItems', data.current.type, itemName, count)
				...
				elseif data.current.type == 'item_weapon' then
					menu.close()
					TriggerServerEvent('cardealer:putStockItems', data.current.type, data.current.value, data.current.ammo)
...
function OpenShopMenuGarage(elements, restoreCoords, shopCoords)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_garage', {
		css = 'unknownstory',
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_garage',
			{
