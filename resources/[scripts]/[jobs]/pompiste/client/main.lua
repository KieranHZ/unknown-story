...
AddEventHandler('esx_pompiste:hasExitedMarker', function(zone)
    ...
    if zone == 'vente' then
        TriggerServerEvent('pompiste:stopVente')
    end
    CurrentAction = nil
    ESX.UI.Menu.CloseAll()
end)
...
function StopNPCJob(cancel)
    if Blips['NPCTargetHouse'] ~= nil then
        RemoveBlip(Blips['NPCTargetHouse'])
        Blips['NPCTargetHouse'] = nil
    end
    OnJob = false
    if cancel then
    ...
    while true do
        ...
        if NPCTargetHouse ~= nil then
            ...
            if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 3 then
                HelpPromt(_U('pickup'))
                if IsControlJustReleased(1, Keys["DELETE"]) and PlayerData.job == 'pompiste' then
                    StopNPCJob()
                    Citizen.Wait(300)
                    Done = false
                end
...
function CreateBlip()
    if PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' and onDuty then
        ...
        BlipVentep = AddBlipForCoord(Config.Zones.vente.Pos.x, Config.Zones.vente.Pos.y, Config.Zones.vente.Pos.z)
        SetBlipSprite(BlipVentep, 18)
        SetBlipColour(BlipVentep, 38)
        SetBlipAsShortRange(BlipVentep, true)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString('Pompiste Vente')
        EndTextCommandSetBlipName(BlipVentep)
        ...
        if BlipVentep ~= nil then
            RemoveBlip(BlipVentep)
            BlipVentep = nil
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' then
                ...
                elseif CurrentAction == 'vente' then
                    TriggerServerEvent('pompiste:vente')
        ...
        if IsControlJustReleased(0, 167) and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' and onDuty then
