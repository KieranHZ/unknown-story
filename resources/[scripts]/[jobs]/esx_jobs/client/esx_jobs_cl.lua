            ...
            function(data, menu)
                ...
                if data.current.value == 'job_wear' then
                    ...
                    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
                        if skin.sex == 0 then
                            TriggerEvent('skinchanger:loadClothes', skin, jobSkin.sj_skin_male)
                        else
                            TriggerEvent('skinchanger:loadClothes', skin, jobSkin.sj_skin_female)
                        end
                    end)
                end
                menu.close()
...
AddEventHandler('esx_jobs:action', function(job, zone)
    ...
    elseif zone.Type == "vehspawner" then
        ...
        for k, v in pairs(Config.Jobs) do
            if PlayerData.second_job.name == k then
...
function refreshBlips()
    ...
    if PlayerData.second_job ~= nil then
        for jobKey, jobValues in pairs(Config.Jobs) do
            ...
            if jobKey == PlayerData.second_job.name then
    ...
    while true do
        ...
        if PlayerData.second_job ~= nil then
            ...
            for k, v in pairs(zones) do
                if onDuty or v.Type == "cloakroom" or PlayerData.second_job.name == "reporter" then
...
Citizen.CreateThread(function()
    while true do
        ...
        if PlayerData.second_job ~= nil and PlayerData.second_job.name ~= 'unemployed' then
            local zones = nil
            local job = nil
            ...
            if zones ~= nil then
                ...
                if IsControlJustReleased(0, Keys["E"]) and not menuIsShowed and isInMarker then
                    if onDuty or zone.Type == "cloakroom" or PlayerData.second_job.name == "reporter" then
                ...
                if isInMarker and not menuIsShowed then
                    hintIsShowed = true
                    if (onDuty or zone.Type == "cloakroom" or PlayerData.second_job.name == "reporter") and zone.Type ~= "vehdelete" then
                        hintToDisplay = zone.Hint
                        hintIsShowed = true
                    elseif zone.Type == "vehdelete" and (onDuty or PlayerData.second_job.name == "reporter") then
                        local playerPed = GetPlayerPed(-1)
                        if IsPedInAnyVehicle(playerPed, 0) then
