		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local element = {
					...
					{ label = 'Dépaneuse poids lourd', value = 'hdwrecker' }
				}

				if Config.EnablePlayerManagement and PlayerData.job and (PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'chief' or PlayerData.job.grade_name == 'experimente') then
					table.insert(element, { label = 'SlamVan', value = 'slamvan3' })
				end
				ESX.UI.Menu.CloseAll()
					...
					if Config.MaxInService == -1 then
						...
						ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
							if isInService then
								...
								ESX.ShowNotification(_U('service_full'))
							end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('mechanic'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
...
AddEventHandler('esx_mechanicjob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'cardealer' and not IsPedInAnyVehicle(playerPed, false) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job and PlayerData.job.name == 'cardealer' then
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job and PlayerData.job.name == 'cardealer' then
			OpenMobileMechanicActionsMenu()
		end

		if IsControlJustReleased(0, Keys['DELETE']) and not isDead and PlayerData.job and PlayerData.job.name == 'cardealer' then
			if NPCOnJob then
				if GetGameTimer() - NPCLastCancel > 0 then
					StopNPCJob(true)
