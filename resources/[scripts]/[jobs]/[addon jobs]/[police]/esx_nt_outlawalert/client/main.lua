...
AddEventHandler('esx_outlawalert:outlawNotify', function(icon, type, title, text)
	if isPlayerWhitelisted then
		Citizen.CreateThread(function()
			Wait(1)
			SetNotificationTextEntry('STRING')
			AddTextComponentString(text)
			SetNotificationMessage(icon, icon, true, type, title)
			...
			if vehicle and ((isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted) then
				...
				ESX.TriggerServerCallback('esx_outlawalert:isVehicleOwner', function(owner)
					if not owner then
						...
						local vehicleLabelText = GetLabelText(vehicleLabel)

						local primary, _ = GetVehicleColours(vehicle)
						local color = colorNames[primary]
						...
						}, streetName, vehicleLabelText, color, playerGender)
					end
				end, plate)
			end
