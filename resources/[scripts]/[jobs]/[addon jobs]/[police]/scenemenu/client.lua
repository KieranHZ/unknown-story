...
Citizen.CreateThread(function()
    ...
    ESX.ShowNotification('GET PLAYERDATA SCENEMENU')
    PlayerData = ESX.GetPlayerData()
end)

_menuPool = NativeUI.CreatePool()
trafficmenu = NativeUI.CreateMenu("Police Traffic")
...
function SpeedZoneSubMenu(menu)
    ...
    zonecreate.Activated = function(sender, item, index)
        ...
        if Config.TrafficAlert then
            local message = "^* ^1Annonce de la circulation: ^r^*^7 La police à ordonné que la circulation sur^2" .. streetName .. " ^7soit de limiter/couper à une vitesse de ^2 " .. speed .. " ^7km / h à la suite d'un incident .. "
        end
        ...
        while true do
            ...
            if IsControlJustPressed(0, Config.ActivationKey) and GetLastInputMethod(0) then
                ...
                elseif Config.UsageMode == "Everyone" then
                    ...
                    if job == 'police' or job == 'army' or 'ambulance' then
                        trafficmenu:Visible(not trafficmenu:Visible())
                    end
                end
