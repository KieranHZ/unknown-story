	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_weaponshop:buyLicense', function(bought)
				if bought then
					ESX.ShowNotification('License achetée')
					menu.close()
				end
			end)
		end
...
AddEventHandler('esx_weaponshop:hasEnteredMarker', function(zone)
	...
	elseif zone == 'PPA' then
		CurrentAction = 'PPA'
		CurrentActionMsg = _U('open_ppa_menu')
		CurrentActionData = { zone = zone }
	end
end)
				...
				if CurrentAction == 'shop_menu' then
					if Config.LicenseEnable then
						ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
							if hasWeaponLicense and Config.Zones[CurrentActionData.zone].Legal then
								OpenShopMenu(CurrentActionData.zone)
							elseif not Config.Zones[CurrentActionData.zone].Legal then
								OpenShopMenu(CurrentActionData.zone)
							else
								ESX.ShowNotification('Vous n\'avez pas de license')
							end
						end, GetPlayerServerId(PlayerId()), 'weapon')
					end
				elseif CurrentAction == 'PPA' then
					ESX.TriggerServerCallback('esx_license:checkLicense', function(hasWeaponLicense)
						if not hasWeaponLicense then
							OpenBuyLicenseMenu(CurrentActionData.zone)
						else
							ESX.ShowNotification('Vous avez déjà une license PPA.')
						end
					end, GetPlayerServerId(PlayerId()), 'weapon')
				end
				CurrentAction = nil
