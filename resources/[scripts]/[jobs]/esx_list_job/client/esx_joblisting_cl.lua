    ...
    ESX.TriggerServerCallback('esx_joblisting:getJobsList', function(data)
        ...
        ESX.UI.Menu.Open(
                ...
                function(data, menu)
                    ...
                    ESX.ShowNotification(_U('La zone de prise de service viens d\'être affichée sur votre GPS'))
                    menu.close()
                end,
                function(data, menu)
                    menu.close()
...
function closeGui()
    ESX.ShowNotification('BEFORE SEND NUI MESSAGE')
    SetNuiFocus(false, false)
    SendNUIMessage({ openMenu = false })
    ESX.ShowNotification('AFTER SEND NUI MESSAGE')
end

RegisterNUICallback('closeMenu', function(data, cb)
    ESX.ShowNotification('Tu as appuyé sur le bouton rouge')
    closeGui()
    cb('ok')
end)
