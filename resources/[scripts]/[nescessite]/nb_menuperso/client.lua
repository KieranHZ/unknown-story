...
function OpenPersonnelMenu()
    ...
    ESX.TriggerServerCallback('NB:getUsergroup', function(group)
        ...
        table.insert(elements, { label = 'Accessoires', value = 'accessories' })
        if (IsInVehicle()) then
        ...
        ESX.UI.Menu.Open(
                ...
                function(data, menu)
                    ...
                    if data.current.value == 'accessories' then
                        TriggerEvent('esx_accessories:getMenuPersoAccessories')
                    end
                    ...
                    if data.current.value == 'menuperso_moi' then
                        ESX.UI.Menu.Open(
                                'default', GetCurrentResourceName(), 'id_card_menu',
                                {
                                    ...
                                    title = 'Me concernant',
                                    elements = {
                                        { label = 'Carte d\'identité', value = 'checkID' },
                                        { label = 'Montrer sa carte d\'identité', value = 'showID' },
                                        { label = 'Permis de conduire', value = 'checkDriver' },
                                        { label = 'Montrer son permis de conduire', value = 'showDriver' },
                                        { label = 'Permis de port d\'armes', value = 'checkFirearms' },
                                        { label = 'Montrer son permis de port d\'armes', value = 'showFirearms' },
                                    }
                                },
                                function(data2, _)
...
function animsAction(animObj)
    if (IsInVehicle()) then
        local source = GetPlayerServerId();
        ESX.ShowNotification("Vous devez sortir de votre véhicule")
    else
        Citizen.CreateThread(function()
