...
Citizen.CreateThread(function()
    while true do
        ...
        if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
            ...
            TriggerEvent('extasie_drug:hasEnteredMarker', currentZone)
        end
        if not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('extasie_drug:hasExitedMarker')
        end
    end
end)

AddEventHandler('extasie_drug:hasEnteredMarker', function(zone)
    if zone == 'Recolte' then
        CurrentAction = 'recolte'
        CurrentActionMsg = _U('recolte')
        CurrentActionData = {}
        ESX.TriggerServerCallback('extasie_drug:inMarker', function(cb)
        end)
...
AddEventHandler('extasie_drug:hasExitedMarker', function()
    if CurrentAction == 'recolte' then
        ESX.TriggerServerCallback('extasie_drug:outOfMarker', function(cb)
            if cb then
                rc = false
            end
        end)
    ...
    while true do
        ...
        if not vending and CurrentAction == 'vente' then
            ...
            if IsControlJustReleased(0, Keys['E']) then
                ESX.TriggerServerCallback('extasie_drug:startVente', function(back)
                    if back then
                        vending = true
                    else
                        vending = false
        ...
        elseif not rc and CurrentAction == 'recolte' then
            ESX.ShowHelpNotification(CurrentActionMsg)
            if IsControlJustReleased(0, Keys['E']) then
                ESX.TriggerServerCallback('extasie_drug:startRecolte', function(back)
                    if back then
                        rc = true
                    else
                        rc = false
        ...
        elseif not treating and CurrentAction == 'traitement' then
            ESX.ShowHelpNotification(CurrentActionMsg)
            if IsControlJustReleased(0, Keys['E']) then
                ESX.TriggerServerCallback('extasie_drug:startTraitement', function(back)
                    if back then
                        treating = true
                    else
                        treating = false
