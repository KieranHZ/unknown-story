...
function showFixePhoneHelper (coords)
    for number, data in pairs(FixePhone) do
        if data.job ~= nil and PlayerData.job ~= nil and data.job == PlayerData.job.name then
    ...
    while true do
        ...
        for i, _ in pairs(PhoneInCall) do
            ...
            if (dist <= soundDistanceMax) then
                ...
                if PhoneInCall[i].job ~= nil and PlayerData.job ~= nil and PhoneInCall[i].job == PlayerData.job.name then
