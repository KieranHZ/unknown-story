...
RegisterServerEvent('esx_truck_inventory:getJobVehicle')
AddEventHandler('esx_truck_inventory:getJobVehicle', function()
    local vehicules = {}
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    MySQL.Async.fetchAll(
            'SELECT * FROM owned_vehicles WHERE job = @job',
            {
            ...
            function(result)
                ...
                TriggerClientEvent('esx_truck_inventory:setJobVehicule', _source, vehicules)
            end)
end)
