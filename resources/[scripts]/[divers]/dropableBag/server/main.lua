...
AddEventHandler('esx-kr-bag:InsertBag', function(source)
    ...
    for i = 1, #xPlayers, 1 do
        TriggerClientEvent('esx-kr-bag:ReSync', xPlayers[i])
    end
...
AddEventHandler('esx-kr-bag:PutItem', function(id, item, label, count, type)
    ...
    MySQL.Async.fetchAll('SELECT * FROM owned_bags WHERE identifier = @identifier ', { ["@identifier"] = identifier }, function(bag)
        if bag[1].itemcount < Config.MaxDifferentItems then
            ...
            elseif type == 'item_account' and count < 1000001 then
            ...
            TriggerClientEvent('esx:showNotification', src, '~r~Vous pouvez loger que' .. Config.MaxDifferentItems .. ' objets différents dans le sac.')
        end
    end)
end)
...
AddEventHandler('esx-kr-bag:PickUpBag', function(id)
    ...
    for i = 1, #xPlayers, 1 do
        ...
        TriggerClientEvent('esx:showNotification', src, 'Appuyez sur ~b~[Y]~w~ pour ouvrir votre sac.')
        TriggerClientEvent('esx-kr-bag:ReSync', xPlayers[i])
    end
end)
...
RegisterServerEvent('esx-kr-bag:deleteBag')
AddEventHandler('esx-kr-bag:deleteBag', function(id)
    local src = source
    MySQL.Sync.execute('DELETE FROM owned_bags WHERE id = @id', { ["@id"] = id })
    MySQL.Sync.execute('DELETE FROM owned_bag_inventory WHERE id = @id', { ["@id"] = id })
    TriggerClientEvent('esx-kr-bag:ReSync', src)
end)
