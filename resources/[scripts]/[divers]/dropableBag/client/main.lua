...
AddEventHandler('esx-kr-bag:insertIntoClient', function()
    ESX.TriggerServerCallback('esx-kr-bag:getAllBags', function(bags)
        for i = 1, #bags, 1 do
...
function Bag()
    ...
    table.insert(elements, { label = 'Déposer le sac', value = 'drop' })
    table.insert(elements, { label = 'Détruire le sac', value = 'delete' })
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lel',
            {
                ...
                elseif data.current.value == 'delete' then
                    ESX.TriggerServerCallback('esx-kr-bag:getBag', function(bag)
                        if bag ~= nil then
                            BagId = bag.bag[1].id
                            TriggerServerEvent('esx-kr-bag:deleteBag', BagId)
                        end
                    end)
...
function TakeItem()
    ...
    ESX.TriggerServerCallback('esx-kr-bag:getBagInventory', function(bag)
        for i = 1, #bag, 1 do
            table.insert(elements, { label = bag[i].label .. ' | x' .. bag[i].count, value = bag[i].item, count = bag[i].count })
        end
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lels',
                {
                    ...
                    elseif data.current.value == 'black_money' then
                        TriggerServerEvent('esx-kr-bag:TakeItem', BagId, data.current.value, data.current.count, "item_account")
                    else
...
function PutItem()
    ...
    ESX.TriggerServerCallback('esx-kr-bag:getPlayerInventory', function(result)
        if result.blackMoney > 0 then
            table.insert(elements, {
                label = 'Argent sale x' .. result.blackMoney,
                --[[                label = _U('dirty_money', ESX.Math.GroupDigits(result.blackMoney)),]]
                count = result.blackMoney,
                name = 'black_money',
                label2 = 'Argent sale'
            })
        end
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lel3',
                {
                    ...
                    elseif data.current.name == 'black_money' then
                        TriggerServerEvent('esx-kr-bag:PutItem', BagId, data.current.name, data.current.label2, data.current.count, "item_account")
                    else
    ...
    while true do
        ...
        for i = 1, #Bags, 1 do
            ...
            if GetDistanceBetweenCoords(playercoords, Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z, true) <= 1.5 and not HasBag then
                wait = 5
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.45, '~g~[E]~w~ pour prendre le sac')
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys['Y']) and HasBag and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsEntityInAir(PlayerPedId()) then
            -- Change F5 to the key you want to open the meny with
            Bag()
        end
    end
end)
