...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedCars', function(source, cb)
    ...
    if Config.DontShowPoundCarsInGarage == true then
        MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE put_by = @owner AND type = @Type AND in_garage_type = @garageType AND `stored` = @stored', {
    ...
    MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE @plate = plate', {
        ...
        if result[1] ~= nil then
            ...
            if originalvehprops.model == vehiclemodel then
                MySQL.Async.execute('UPDATE owned_vehicles SET vehicle = @vehicle, in_garage_type = @garageType, put_by = @owner WHERE plate = @plate', {
                    ['@owner'] = GetPlayerIdentifiers(source)[1],
...
ESX.RegisterServerCallback('esx_advancedgarage:getCountOwnedCars', function(source, cb, owner)
    local result = MySQL.Sync.fetchScalar('SELECT count(1) FROM owned_vehicles WHERE put_by = @owner AND job IS NULL AND stored = 1', {
        ['@owner'] = owner
    })
    cb(result)
end)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedCars', function(source, cb)
    ...
    MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE put_by = @owner AND type = @Type AND job IS NULL AND `stored` = @stored', {
        ['@owner'] = GetPlayerIdentifiers(source)[1],
        ['@Type'] = 'car',
        ['@stored'] = false
    }, function(data)
...
-- Fetch Pounded Job Vehicles
ESX.RegisterServerCallback('esx_advancedgarage:getOutJobCars', function(source, cb, xPlayer)
    local jobCars = {}
...
-- Check Money for Pounded Job cars
ESX.RegisterServerCallback('esx_advancedgarage:checkMoneyJob', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.get('money') >= Config.PolicingPoundPrice then
        cb(true)
    else
...
AddEventHandler('esx_advancedgarage:payAircraft', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    TriggerEvent('esx_society:getSociety', 'police', function(police)
        if police ~= nil then
            -- verified society
            TriggerEvent('esx_addonaccount:getSharedAccount', police.account, function(policeAccount)
                -- does the society money to pay its employees?
                policeAccount.addMoney(Config.AircraftPoundPrice)
...
AddEventHandler('esx_advancedgarage:payBoat', function()
    ...
    TriggerEvent('esx_society:getSociety', 'police', function(police)
        if police ~= nil then
            ...
            TriggerEvent('esx_addonaccount:getSharedAccount', police.account, function(policeAccount)
                -- does the society money to pay its employees?
                policeAccount.addMoney(Config.BoatPoundPrice)
...
AddEventHandler('esx_advancedgarage:payCar', function()
    ...
    TriggerEvent('esx_society:getSociety', 'police', function(police)
        if police ~= nil then
            ...
            TriggerEvent('esx_addonaccount:getSharedAccount', police.account, function(policeAccount)
                -- does the society money to pay its employees?
                policeAccount.addMoney(Config.CarPoundPrice)
...
-- Pay for Pounded Job
RegisterServerEvent('esx_advancedgarage:payJob')
AddEventHandler('esx_advancedgarage:payJob', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    xPlayer.removeMoney(Config.JobPoundPrice)
    TriggerClientEvent('esx:showNotification', source, _U('you_paid') .. Config.JobPoundPrice)
end)
...
AddEventHandler('esx_advancedgarage:payhealth', function(price)
    ...
    TriggerEvent('esx_addonaccount:getSharedAccount', 'society_cardealer', function(tsAccount)
        if tsAccount ~= nil then
            tsAccount.addMoney(price)
        end
    end)
