...
function OpenAdminActionMenu(player)
    ...
    for i=1, #data.accounts, 1 do
      ...
      if data.dob ~= nil then
        ...
        dobLabel = 'Date de naissance : absence de données'
      end

      if data.height ~= nil then
        heightLabel = 'Taille : ' .. data.height
      else
        heightLabel = 'Taille : absence de données'
      end

      if data.name ~= nil then
        idLabel = 'Steam ID : ' .. data.name
      else
        idLabel = 'Steam ID : absence de données'
      end
	  
      local elements = {
        {label = 'Nom/Prenom: ' .. data.firstname .. " " .. data.lastname, value = nil},
        {label = 'Argent liquide: '.. data.money, value = nil},
        {label = 'Argent banque: '.. data.bank, value = nil},
...
-- taken from Easy Admin (thx to Bluethefurry)  --
			...
			if targetGod then
				table.insert(text,"~g~Anticheat : Godmode détecté")
			else
				table.insert(text,"~r~Anticheat : Godmode non détecté")
			end
			if not CanPedRagdoll(targetPed) and not IsPedInAnyVehicle(targetPed, false) and (GetPedParachuteState(targetPed) == -1 or GetPedParachuteState(targetPed) == 0) and not IsPedInParachuteFreeFall(targetPed) then
				table.insert(text,"~g~Anticheat : Anti-chute détecté")
			end
			-- health info
			table.insert(text,"Vie restante"..": "..GetEntityHealth(targetPed).."/"..GetEntityMaxHealth(targetPed))
