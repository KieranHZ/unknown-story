...
Citizen.CreateThread(function()
    while true do
        ...
        if (IsPedJacking(GetPlayerPed(-1))) then
            if (Config.settings.LogPedJacking == true and isJacking) then
                local playerPed = GetPlayerPed(-1)
                local coords = GetEntityCoords(playerPed)
...
Citizen.CreateThread(function()
    while true do
        ...
        if (IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
            if (Config.settings.LogEnterPoliceVehicle == true and not isIncarPolice and PlayerData.job.name ~= "police") then
...
Citizen.CreateThread(function()
    while true do
        ...
        if (IsPedInAnyVehicle(GetPlayerPed(-1)) and not IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
            if (Config.settings.LogEnterPoliceVehicle == true and not isIncar) then
