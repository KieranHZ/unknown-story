...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	if props.color1 ~= nil then
		...
		TriggerServerEvent('gar:writemsg', 'Je set la couleur color1:', props.color1)
	end

	if props.color2 ~= nil and props.color1 ~= nil then
...
ESX.ShowInventory = function()
	...
	for i=1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].money > 0 and ESX.PlayerData.accounts[i].name ~= 'bank' then
			local formattedMoney = _U('locale_currency', ESX.Math.GroupDigits(ESX.PlayerData.accounts[i].money))
			table.insert(elements, {
				label     = ('%s: <span style="color:red;">%s</span>'):format(ESX.PlayerData.accounts[i].label, formattedMoney),
