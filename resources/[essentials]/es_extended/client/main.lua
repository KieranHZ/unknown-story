...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	...
	if ESX.PlayerData.lastPosition then
		ESX.ShowNotification('Position set')
	end
...
AddEventHandler('esx:pickupWeapon', function(weaponPickup, _, ammo)
	local playerPed = PlayerPedId()
		...
		while true do
			...
			for i = 1, #players, 1 do
				if players[i] ~= PlayerId() then
					...
					local _ = CreateMpGamerTag(ped, ('·'), false, false, '', false)
				end
			end
		end
	end)
...
Citizen.CreateThread(function()
	while true do
		...
		for _,v in pairs(pickups) do
			...
			local _, closestDistance = ESX.Game.GetClosestPlayer()
