function CreateExtendedPlayer(player, accounts, inventory, job, loadout, name, lastPosition)
	...
	self.setJobSalary = function(job, grade, salary)
		grade = tostring(grade)
		local lastJob = json.decode(json.encode(self.job))
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('es_extended: ignoring setJobSalary for %s due to job not found!'):format(self.source))
		end
	end
